...
type precision_literals =
  { max_int_z : Z.t option;
    (* [None] when [sb > emax], the formula for [max_int] is only meaningful
       when [sb <= emax]. *)
    pow2sb : Z.t;
    half_pow2sb : Z.t;
...
let opt_literal_fact fact eb_t sb_t name literal_o acc =
  match literal_o with
  | Some literal -> fact eb_t sb_t name literal acc
  | None -> acc
...
let type_literal_facts env eb sb =
  ...
  |> opt_literal_fact int_literal_fact eb_t sb_t E.FP.Names.max_int c.max_int_z
  |> opt_literal_fact real_literal_fact eb_t sb_t E.FP.Names.max_real
       c.max_int_z
