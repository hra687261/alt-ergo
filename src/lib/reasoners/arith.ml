...
let calc_power (c : Q.t) (d : Q.t) (ty : Ty.t) =
  (* d must be integral and if we work on integer exponentation, d must be
     positive*)
...
struct
  ...
  module Debug = struct
    let solve_aux r1 r2 =
      if Options.get_debug_arith ()
      then
        Printer.print_dbg ~module_name:"Arith" ~function_name:"solve_aux"
    ...
    let solve_one r1 r2 sbs =
      ...
      if Options.get_debug_arith ()
      then
        Printer.print_dbg ~module_name:"Arith" ~function_name:"solve_one"
          "solving %a = %a yields:@,%a" X.print r1 X.print r2
          (Printer.pp_list_no_space print)
          sbs
  end
  (*BISECT-IGNORE-END*)
  ...
  (* Add range information for [t = t1 / t2], where `/` is euclidean division.
     ...
     Requires: [t], [t1], [t2] have type [Tint] Requires: [p2] is the term
     representative for [t2] Requires: [p2] is a non-zero constant *)
  ...
  (* Compute the remainder of euclidean division [t1 % t2] as [t1 - t2 * (t1 /
     t2)], where `a / b` is euclidean division.

     Requires: [t1], [t2] have type [Tint] Requires: [p1] is the representative
     for [t1] Requires: [p2] is the representative for [t2] Requires: [p2] is a
     non-zero constant *)
  ...
  let exact_sqrt_or_Exit q =
    (* this function is probably not accurate because it works on Z.t to compute
       eventual exact sqrt *)
    let c = Q.sign q in
  ...
  let rec mke coef p t ctx =
    ...
    | Sy.Op Sy.Div, [t1; t2] ->
      ...
      if
        Options.get_no_nla ()
        && (P.is_const p2 == None || (ty == Ty.Tint && P.is_const p1 == None))
      then
        (* becomes uninterpreted *)
        ...
        let p3, ctx =
          ...
          | p, approx -> if approx then mk_euc_division t t1 p2 ctx else p, ctx
    ...
    | Sy.Op Sy.Modulo, [t1; t2] ->
      ...
      let p2, ctx = mke Q.one (empty_polynome ty) t2 ctx in
      if
        Options.get_no_nla () && (P.is_const p1 == None || P.is_const p2 == None)
      then
        (* becomes uninterpreted *)
        let tau = E.mk_term (Sy.name ~ns:Internal "@%") [t1; t2] ty in
        let xtau, ctx' = X.make tau in
  ...
  let make t =
    ...
    | { f = Op (Int2BV _); _ } -> X.term_embed t, []
    | _ ->
  ...
  let mod_sym a b =
    ...
    let m =
      if Q.sign m < 0
      then if Q.compare m (Q.minus b) >= 0 then Q.add m b else assert false
      else if Q.compare m b <= 0
      then m
      else assert false
    in
  ...
  let subst_bigger x l =
    List.fold_left
      (fun (l, sb) (b, y) ->
        if X.ac_extract y != None && X.str_cmp y x > 0
        then
  ...
  let extract_min = function
    ...
    | (a, x) :: s ->
      List.fold_left
        (fun ((a, x), l) (b, y) ->
          if Q.compare (Q.abs a) (Q.abs b) <= 0
          then (a, x), (b, y) :: l
          else (b, y), (a, x) :: l)
        ((a, x), [])
        s
  ...
  let rec omega l b =
    ...
    (* 2. substituer les aliens plus grand que x pour assurer l'invariant sur
       l'ordre AC *)
    ...
    then
      ...
      let a, l, b =
        if Q.sign a < 0
        then Q.minus a, List.map (fun (a, x) -> Q.minus a, x) l, Q.minus b
        else a, l, b
      in
  ...
  and omega_sigma sbs a x l b =
    ...
    (* 3.1. Attention au signe de b : on le passe a droite avant de faire
       mod_sym, d'ou Q.minus *)
    let b_mod = Q.minus (mod_sym (Q.minus b) m) in
    let p = P.create l_mod b_mod Ty.Tint in
    let sbs = (x, p) :: sbs in
    (* 4. on substitue x par sa valeur dans l'equation de depart. Voir la
       formule (5.64) *)
  ...
  let is_null p =
    if Q.sign (snd (P.separate_constant p)) <> 0 then raise Util.Unsolvable;
    []

  let solve_int p = try solve_int p with Not_found -> is_null p
  ...
  let make_idemp _ _ sbs lvs unsafe_mode =
    ...
    (* This assert is not TRUE because of AC and distributivity of '*'
       Options.heavy_assert (fun () -> X.equal (apply_subst a sbs) (apply_subst
       b sbs)); *)
    List.iter
      (fun (p, _) ->
        if not (SX.mem p lvs)
  ...
  let solve r1 r2 pb =
    ...
    try
      if Options.get_debug_arith ()
      then
        Printer.print_dbg ~module_name:"Arith" ~function_name:"solve"
  ...
  let fully_interpreted sb =
    match sb with Sy.Op (Sy.Plus | Sy.Minus) -> true | _ -> false
  ...
  (* this function is only called when some arithmetic values do not yet appear
     in IntervalCalculus. Otherwise, the simplex with try to assign a value *)
  let assign_value =
