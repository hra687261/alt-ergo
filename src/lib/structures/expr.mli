...
type term_view = private
  { f : Symbols.t;
    ...
    vars : (Ty.t * int) Var.Map.t;
        (** Map of free term variables in the term to their type and number of
            occurrences. *)
    vty : Ty.TvSet.t;  (** Map of free type variables in the term. *)
    depth : int;
    nb_nodes : int;
    pure : bool;
...
and quantified = private
  { name : string;  (** Name of the lemma. This field is used by printers. *)
    main : t;  (** The underlying formula. *)
    toplevel : bool;
        (** Determine if the quantified formula is at the top level of an
            asserted formula.

            An {e asserted formula} is a formula introduced by [(assert ...)] or
            generated by a function definition with [(define-fun ...)].

            By {e top level}, we mean that the quantified formula is not a
            subformula of another quantified formula.

            For instance, the subformula ∀y:int. ¬G(y) of the asserted formula
            ¬(∀y:int. ¬G(y)) is also considered at the top level.

            Notice that quantifiers of the same kind are packed as much as
            possible. For instance, if we assert the formula: ∀α. ∀x:list α.
            ∃y:α. F(x, y) Then the two universal quantifiers are packed in a
            same top level formula but the subformula ∃y:α. F(x, y) is not at
            the top level.

            This flag is important for the prenex polymorphism.
            - If this flag is [true], all the free type variables of [main] are
              implicitely considered as quantified.
            - Otherwise, the free type variables of the lemma are the same as
              the underlying formula [main] and are stored in the field
              [sko_vty]. *)
    ...
    sko_v : t list;
        (** Set of all the free variables of the quantified formula. In other
            words, this set is always the complementary of [binders] in the set
            of free variables of [main].

            The purpose of this field is to retrieve these variables quickly
            while performing the lazy skolemization in the SAT solver (see
            [skolemize]). *)
    sko_vty : Ty.t list;
    ...
    loc : Loc.t;
        (** Location of the "GLOBAL" axiom containing this quantified formula.
            It forms with name a unique identifier. *)
...
(** [pp_smtlib ppf e] prints the expression [e] on the formatter [ppf] using the
    SMT-LIB standard. *)
...
(** Special names used for AC(X) abstraction. These corresponds to the K sort in
    the AC(X) paper. *)
...
(** [mk_abstract ty] creates an abstract model term of type [ty]. This function
    is intended to be used only in models. *)
...
(** [mk_constr c args ty] converts the Dolmen constructor [c] into an expression
    with arguments [args].

    @raise Invalid_argument if [c] is not a Dolmen constructor.

    @raise Invalid_argument
      if no order have been attached to [c] with [Nest.attach_orders]. *)
...
(** [sub_term acc e] returns the acc augmented with the term and all its
    sub-terms. Returns the acc if e is not a term. Does not go through literals
    (except positive uninterpreted predicates application) and formulas *)
...
(** returns the maximal terms of the given literal. Assertion failure if not a
    literal (should replace the assertion failure with a better error) **)

val max_ground_terms_of_lit : t -> Set.t
(** returns the maximal ground terms of the given literal. Assertion failure if
    not a literal **)

val atoms_rec_of_form : only_ground:bool -> t -> Set.t -> Set.t
(** traverses a formula recursively and collects its atoms. Only ground atoms
    are kept if ~only_ground is true *)
...
(** [make_triggers f binders decl menv] generate multi-triggers for the formula
    [f] and binders [binders].

    An {e escaped variable} of a pattern is a variable that is not in [binders]
    (but the variable is bound by an inner quantified formula). We can replace
    escaped variables by the underscore variable [Var.underscore].

    For instance, if [binders] is the set [{x, y}] and [g(x, y, z)] is a pattern
    where [{(x, y, z)}] are free term variables, we can replace [z] by [_].
    ...
    The matching environment [env] is used to limit the number of multi-triggers
    generated per axiom. *)
...
val mk_forall :
  ...
  (* quantified formula *)
  toplevel:bool ->
  (* for future triggers computation in presence of vty *)
  decl_kind:decl_kind ->
  t
...
val mk_exists :
  ...
  (* quantified formula *)
  toplevel:bool ->
  (* for future triggers computation in presence of vty, and to construct a
     toplevel forall that cover vtys *)
  decl_kind:decl_kind ->
  t
...
(** [is_model_term e] checks if the expression [e] is a model terms. A model
    term can be:
